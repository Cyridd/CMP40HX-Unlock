...
func knownIssuesBlock() string {
	...
	sb.WriteString("· 寨板/多卡: retrain-only 在部分主板/拓扑训不上 Gen2, 需 Root Link Disable 回退(瞬断链路)。v2.6 登录任务默认已自动尝试一次 Stage2(Gen2AutoHard); 仍失败可手动 `40HXInstaller.exe -gen2 -hard`。\n")
	sb.WriteString("· EFI 找不到卡(旧版只扫 bus 0-7): v3.0 已扩到 0-16 + CF8 全 0-255 兜底, 覆盖 AGESA/高总线(微星 B450 实测 bus 0x10); 若重装 v3.0 EFI 后仍 not found, 多为固件没初始化该无头槽 — 走 BIOS: Above4G+Re-Size BAR / Init Display First=PEG / 插 CPU 顶槽, 并把 40hx_log.txt(含 \"diag: CF8 visible devices\" 设备映射段)发作者。\n")
	...
	sb.WriteString("· 报告 issue 时请附: 本诊断 txt + %TEMP%\\40HX_installer.log + 主板/CPU/GPU/系统版本。\n")
	return sb.String()
}
...
func check() {
	...
	w("Gen2 任务: %s\n",
		map[bool]string{true: "已注册 (" + taskStatus + ", 上次结果: " + taskResult + ")",
			false: "未注册 (修复: 右键管理员运行 40HXInstaller.exe -task)"}[taskOK])
			...
			if !drvOK {
				if !isAdmin() {
					...
					if ever, ev := drvLogEvidence(); ever {
						...
						verdict = ">>> 驱动不可用 — 请从 40HXUnlock 发布目录(含 gen2\\drivers)运行, 或先管理员重跑 40HXInstaller.exe 安装驱动"
					}
				} else {
	...
	if !gsOn {
		tips = append(tips, "· GSP 未启用: 双击 40HXInstaller.exe → ① 勾 [GSP 启用] 点[安装所选组件]")
	}
	if gpuOK && st.SS0OK && !st.Unlocked {
		tips = append(tips, "· EFI 算力解锁未生效(SS0 锁定): 若本机未部署/已卸载 '40HX Unlock' 解锁 EFI, 此提示属预期, 算力会保持锁定 — 想恢复请重跑 40HXInstaller.exe 勾选[算力 EFI 部署+固件启动项]; 若 EFI 已装, 则确认开机走了 '40HX Unlock' 启动项 / Above 4G 已开 / Secure Boot 已关")
	}
	...
	if st.SS0OK && st.Unlocked && st.Speed < 2 && st.TLS >= 2 {
		tips = append(tips, "· Gen2 目标已配置(TLS=Gen2)但当前链路 Gen1: 多为空闲省电降速(正常, 负载自动回升); 若持续负载仍 Gen1: 直接在 GUI ② 点[立即执行 Gen2] 重训（Gen2AutoHard 默认开会自动走 Stage2 回退）; 仅当 ② 关掉自动回退或想强制触发时, 再用命令行 `40HXInstaller.exe -gen2 -hard`(瞬断链路, 见 README §3.3)")
	}
	...
	if !drvOK {
		...
		if ever, ev := drvLogEvidence(); ever {
			...
			tips = append(tips, "· 驱动从未成功部署(无 installer.log/gen2_status 成功记录): 双击 40HXInstaller.exe → ① 区点[一键完整安装]完成部署与自启注册")
		}
	}
	if len(tips) > 0 {
		w("\n建议:\n%s\n", strings.Join(tips, "\n"))
