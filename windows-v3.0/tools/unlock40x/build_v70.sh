...
echo "=== 3. link ==="
# --image-base 0x0  ← ключевой фикс:
#   ld -mi386pep по умолчанию ставит базу 0x140000000.
#   При такой базе .bss попадает за 2 ГБ, и R_X86_64_32S-перемещения
#   (знаковые 32-бит, которые gcc генерирует для статических переменных
#   в small code model) переполняются → "перемещение обрезано".
#   EFI-образ всё равно реолоцируется прошивкой через .reloc-секцию,
#   поэтому 0x0 абсолютно безопасен и является стандартной практикой
#   для gnu-efi сборок.
ld -mi386pep --subsystem 10 --image-base 0x0 -e u40x_entry -o "$OUT" \
   $OBJS "$EFI_LIB/libefi.a" 2>&1 | tail -20
# Проверяем статус ld через PIPESTATUS (|| true скрывал ошибки линкера)
LD_STATUS=${PIPESTATUS[0]}
if [ "$LD_STATUS" -ne 0 ]; then
  echo "LINK FAILED (ld exit $LD_STATUS)"
  echo DONE
  exit 1
fi
