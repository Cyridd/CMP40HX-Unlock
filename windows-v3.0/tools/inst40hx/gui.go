...
func scanStatus() []statusItem {
	...
	items = append(items, statusItem{"40HX 显卡", gpuOK,
		map[bool]string{true: "已检测到 (VEN_10DE&DEV_1F0B)", false: "未检测到 — 确认插好且驱动已装"}[gpuOK]})
	...
	items = append(items, statusItem{"ESP 解锁 EFI", espEFI,
		map[bool]string{true: "\\EFI\\40HX\\40HXUNLK.EFI 已部署", false: "未部署 (Legacy 机器属预期)"}[espEFI]})
	...
	items = append(items, statusItem{"Gen2 Run 键兜底", rkOK,
		map[bool]string{true: "已写入 (40HXGen2)", false: "未写入"}[rkOK]})
...
func (st *guiState) envGuide(items []statusItem) string {
	...
	if it, ok := m["固件启动项"]; ok && !it.ok {
		if bl, ok2 := m["引导模式"]; !ok2 || bl.ok {
			if strings.Contains(it.note, "不在") {
				g = append(g, "· 启动项存在但不在 displayorder 首位: 进 BIOS 把 '40HX Unlock' 设为第一启动项(否则开机可能不执行)")
			} else {
	...
	if it, ok := m["Gen2 登录任务"]; ok && !it.ok {
		g = append(g, "· Gen2 登录自启未注册: 勾选[Gen2 登录自启]安装; 或管理员运行 40HXInstaller.exe -task")
	}
...
func (st *guiState) scanOnce() {
	...
	if avKey != st.lastAV {
		if len(av) > 0 {
			...
			fmt.Println("        %ProgramData%\\40HXUnlock\\drivers\\ 下的 ThrottleStop.sys / WinRing0x64.sys")
		}
		st.lastAV = avKey
	}
}
...
func (st *guiState) applySmartDefaults() {
	...
	if !flags["40HX 显卡"] {
		fmt.Println("[i] 未检测到 40HX — 安装区保持全不勾(请先确认显卡/驱动)")
		st.sync(func() {
...
func (st *guiState) printDefErr(prefix string, err error) {
	...
	if errors.Is(err, hxcore.ErrMpUnavailable) {
		...
		fmt.Println("      %ProgramData%\\40HXUnlock\\drivers\\ 下同名两个 .sys")
		return
	}
...
func runGUI() {
	...
	if createErr != nil {
		msgbox("40HX 安装器", "GUI 初始化失败: "+createErr.Error()+"\n请使用命令行方式 (40HXInstaller.exe -h)。", mbIconError)
		return
	}
	st.log.mw = st.mw
	st.log.te = st.teLog
...
func (st *guiState) installSelected(sel map[string]bool) {
	...
	if sel["task"] {
		...
		if err := setupGen2Task(); err != nil {
			fmt.Println("  [!] 登录自启注册失败:", err)
			fmt.Println("  [!] 可稍后以管理员运行: 40HXInstaller.exe -task")
		}
	}
